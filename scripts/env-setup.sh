...
# Config goes into: /usr/local/etc/scanoss/licenses
# Logs go into: /var/log/scanoss/licenses
...
export CONFIG_DIR="${BASE_C_PATH}/licenses"
export LOG_DIR=/var/log/scanoss
export L_PATH="${LOG_DIR}/licenses"
export DB_PATH_BASE=/var/lib/scanoss
export SQLITE_PATH="${DB_PATH_BASE}/db/sqlite/licenses"
export SQLITE_DB_NAME=base.sqlite
export TARGET_SQLITE_DB_NAME=db.sqlite
export CONF_DOWNLOAD_URL="https://raw.githubusercontent.com/scanoss/licenses/refs/heads/main/config/app-config-prod.json"
...
SC_SERVICE_FILE="scanoss-licenses-api.service"
SC_SERVICE_NAME="scanoss-licenses-api"
if [ -n "$ENVIRONMENT" ] ; then
  SC_SERVICE_FILE="scanoss-licenses-api-${ENVIRONMENT}.service"
  SC_SERVICE_NAME="scanoss-licenses-api-${ENVIRONMENT}"
fi
...
if ! cp scanoss-licenses-api.sh /usr/local/bin ; then
  echo "Error: licenses startup script copy failed."
  exit 1
fi
####################################################
#                SEARCH CONFIG FILE                #
...
BINARY=scanoss-licenses-api
if [ -f $BINARY ] ; then
  echo "Copying app binary to /usr/local/bin ..."
  if ! cp $BINARY /usr/local/bin ; then
    echo "Error: copy $BINARY failed."
    echo "Please make sure the service is stopped: systemctl stop scanoss-licenses-api."
    exit 1
  fi
else
